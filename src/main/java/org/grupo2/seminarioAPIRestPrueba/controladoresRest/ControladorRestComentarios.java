...
public class ControladorRestComentarios {
    ...
    public void crearComentarioUniversidad(@RequestParam(name = "alias") String alias,
                                           ...
                                           @RequestParam(name = "punctuation") int punctuation) throws UniversidadException, CarreraException, UsuarioException, PuntuacionesException {
        ControladorComentarios.getInstance().crearComentarioUniversidad(alias, rol, idUniversidad, comentario, punctuation);
    }
    ...
    @PostMapping("/aprobarComentarioUniversidad")
    @CrossOrigin(origins = "http://localhost:3000/")
    public void aprobarComentarioUniversidad(@RequestParam(name = "idComentario") int idComentario) throws ComentariosException, UniversidadException, PuntuacionesException {
        ControladorComentarios.getInstance().aprobarComentarioUniversidad(idComentario);
    }
