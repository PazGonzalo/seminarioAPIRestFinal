...
public class ControladorRestCarreras {
    ...
    public void crearCarrera(@RequestParam(name = "idUniversidad") int idUniversidad,
                             ...
                             @RequestParam(name= "descAmigable") String descAmigable,
                             @RequestParam(name = "perfilProfesional") String perfil) throws UniversidadException, CarreraException {
        ControladorCarreras.getInstance().crearCarrera(idUniversidad, nombre, descripcion, perfil, duracion,modalidad,mañana,tarde,noche,descAmigable);
    }
    ...
    @RequestMapping("/getCarrerasDistintasComparacionById")
    @CrossOrigin(origins = "http://localhost:3000/")
    public List<CarreraVO> obtenerCarrerasDistintasByIDParticular(@RequestParam(name = "carreras") String carreras) throws CarreraException {
        return ControladorCarreras.getInstance().obtenerCarrerasDistintasByString(carreras);
    }
